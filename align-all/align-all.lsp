...
;;(setq BGhangju nil)
(defun align-all:entity ( / *error* a all b c e hangju1 hangju2 l m n p snap x x0 xyz xyz_new y y0 z)
  ...
  (if (setq a (ssget (list (cons 0 "*TEXT,DIMENSION,INSERT,ATTDEF,ACAD_TABLE,TCH_ELEVATION")))) 
	...
	(setvar "osmode" 16383)
	(setq hangju2 (getdist (strcat "\n输入行距 支持鼠标点选 <" (rtos (@:get-config 'align-all:hangju) 2 2) ">："))) 
