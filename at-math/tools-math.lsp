...
(defun @m:text-sum ( / s1 si% ti% ename total )
  ...
  (setq s1 (ssget '((-4 . "<OR") (0 . "text")(0 . "tch_text")(-4 . "OR>" ))))
  ;;(foreach si% s1
  (setq ti% 0)(setq total 0)
  (if (/= s1 nil)
      (while (<= ti% (- (sslength s1) 1))
	(setq ename (ssname s1 ti%))
	(if (null (or (member '(0 . "text") (entget ename))
		      (member '(0 . "tch_text") (entget ename))
		      ))
	    (setq total (+ total (atof (cdr (assoc 1 (entget ename)))))))
	(setq ti% (+ 1 ti%))))
  (setq @m:*result* total)
  (print total)
  (princ)
...
(defun @m:avg ( / s1  ti% ename total )
  ...
  (if (/= s1 nil)
	...
	(if (null (or (member '(0 . "text") (entget ename))
	    ...
	    (setq total (+ total (atof (cdr (assoc 1 (entget ename)))))))
	(setq ti% (+ 1 ti%))))
  (setq @m:*result* (/ total (sslength s1)))
  )
(defun @:probe-int (str)
  "分析一个字符串数值，如果是整数则取整。"
...
(defun @m:sx ( / minnum1 s1 ti% ename e )
  ...
  (if (/= s1 nil)
	...
	(setq ename (ssname s1 ti%))
	(setq e (entget ename))
	(if (and (> (ascii (cdr (assoc 1 e))) 48)
		 (< (ascii (cdr (assoc 1 e))) 57))
	    (if  (< (atoi (cdr (assoc 1 e ))) minnum1) 
		(changecolor e 141)
	      (changecolor e 1))
	  )
	(setq ti% (+ 1 ti%))
