			...
			(command "-style" sty ttf))
			  ...
			  (while (not (zerop (getvar "cmdactive")))
				...
				(command-s "")
                  ...
                  (progn (setq thisdrawing (vlax-get (vlax-get-acad-object) 'activedocument)
                               textstyles (vlax-get thisdrawing 'textstyles)
...
(defun fonts:fix-fonts ( tips / old_regenmode winfonts
  ...
  (vlax-for st (vlax-get (vlax-get (vlax-get-acad-object) 'activedocument) 'textstyles)
            (setq styn (strcase (vlax-get st 'name))
		  name1 (strcase (vlax-get st 'fontfile))
		  name2 (strcase (vlax-get st 'BigFontFile))
		  )
            (or (= (substr styn 1 1) "*")
		(= styn "")
		(wcmatch styn "*|*")
