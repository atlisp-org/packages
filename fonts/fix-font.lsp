...
(defun fonts:fix-fonts (tips / old_regenmode winfonts stylen TS_eng TS_chn TS_ttf shx1 shx2 chk1 styn name0 name1 name2 
  ...
  (foreach st fonts 
    ...
    (if (wcmatch name1 "*.TTF,*.TTC") 
      (progn 
        (cond 
          ((member name1 m4)) ;之前已检查，字库存在
          ((member name1 m3) (setq chk1 T)) ;之前已检查，字库不存在
          ((or 
             (findfile name1) ;在acad路径内找到字库
      ...
      (progn 
        ...
        (cond 
          ((member name1 m4) (setq shx1 name1)) ;之前已检查，字库存在
          ((member name1 m1)
           (setq shx1 ts_eng
                 chk1 T)) ;之前已检查，字库不存在
          ((= "" name1)
          ...
          ((findfile name1)
           (setq shx1 name1
                 m4   (cons name1 m4))) ;检查，字库存在
          (T
           (setq shx1 ts_eng
                 m1   (if (member name1 m1) m1 (cons name1 m1)))) ;检查，字库不存在
        )
        (cond 
          ((= name2 "")) ;没设置
          ...
          ((findfile name2)
           (setq shx2 name2
                 m4   (cons name2 m4))) ;检查，字库存在
          (T
           (setq chk1 T
                 shx2 ts_chn
                 m2   (cons name2 m2))) ;检查，字库不存在
        )
