...
(defun mark-order:findtext () 
  ...
  (setq txts (vl-remove-if-not 
		  ...
		  ((= "TCH_OPENING"  (entity:getdxf txt 0))
		   (mark-order:check-text txt 302) ;; (entity:getdxf txt 302))
		   )
                  ((/= "INSERT" (entity:getdxf txt 0)) (mark-order:check-text txt 1))
		   ...
		   ;; attributes
		   (or flag
		       (apply 'or (mapcar 'check-str (mapcar 'cdr (block:get-attributes txt))))
		       )
                   )
                  )
                 )
...
(@:add-menu (_"Demo") (_"Number by string") '(mark-order:main (mark-order:findtext)))
